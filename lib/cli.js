...
const { orchestrateDocs } = require("./orchestrate");
...
Usage:
  ...
  dreampia-dev-kit orchestrate [--mode beginner|expert] <file-or-dir...>
  dreampia-dev-kit doc-ops [--mode beginner|expert] <file-or-dir...>
...
Commands:
  ...
  orchestrate          Create a read-only documentation operation queue and next prompts.
  doc-ops              Alias for orchestrate.
...
function parseOrchestrateArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--mode") {
      const value = args[index + 1];
      if (!value || !["beginner", "expert"].includes(value)) {
        fail("--mode must be one of beginner, expert");
      }
      mode = value;
      index += 1;
      continue;
    }
    if (arg.startsWith("-")) fail(`Unknown orchestrate option: ${arg}`);
    inputs.push(arg);
  }

  return {
    files: expandInputs(inputs, fail),
    mode,
    targets: inputs,
  };
}
...
function runOrchestrate(args) {
  const { files, mode, targets } = parseOrchestrateArgs(args);
  ensureFiles(files);
  return orchestrateDocs(files, { mode, targets }, fail);
}
...
function main(args) {
  ...
  if (command === "orchestrate" || command === "doc-ops") process.exit(runOrchestrate(rest));
