...
```text
idea -> guide -> init/doc-pack -> validate -> orchestrate/doc-ops -> explain -> fix docs -> implement -> revalidate
```
...
```bash
npx dreampia-dev-kit orchestrate docs/
npx dreampia-dev-kit doc-ops --mode expert docs/
```

역할:

- 현재 문서팩 상태를 읽고 다음 문서 운영 작업을 정한다.
- required, recommended, verify, next operation queue를 보여준다.
- 구현 준비 상태를 명확히 말한다.
- Codex와 Claude Code에 넣을 다음 prompt를 생성한다.
- 기본값은 read-only다.
...
- `orchestrate`, `doc-ops`로 read-only 문서 운영 큐와 다음 agent prompt 제공
...
- `related_docs` 기반 drift 근거 강화
- operation queue dependency ordering
- 자동화용 JSON output 검토
- Codex/Claude Code skill routing prompt 고도화

## 성공 기준

- 사용자가 "Dreampia는 문서 버전의 OMC/LazyCodex"라고 이해할 수 있다.
- 하지만 Dreampia가 구현 자동화가 아니라 문서 운영과 검증을 맡는다는 경계도 분명하다.
- 초보자가 `guide -> init/doc-pack -> validate -> orchestrate -> explain -> fix` 루프를 혼자 돌릴 수 있다.
