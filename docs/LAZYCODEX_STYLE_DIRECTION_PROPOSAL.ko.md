...
다만 LazyCodex를 그대로 따라가면 안 된다. LazyCodex는 Codex를 위한 agent harness다. 프로젝트 메모리, 계획, 실행, 검증, hooks, model routing, sub-agent role까지 다룬다. Dreampia가 이 영역에서 정면 경쟁하면 범위가 너무 커지고, 지금 가진 강점인 문서 품질 레이어가 흐려진다.
...
즉, 사용자는 LazyCodex처럼 쉽게 설치하고, 명령 몇 개만 기억하고, 문제가 생기면 doctor/explain으로 고칠 수 있어야 한다. 하지만 Dreampia가 책임지는 것은 구현 오케스트레이션이 아니라 PRD, TRD, IA, user-flow, API spec, ERD, QA, doc-audit가 연결된 문서 시스템이어야 한다.
...
- LazyCodex처럼 full agent harness가 되려고 하지 않는다.
...
```text
Dreampia is the documentation operating layer for vibe coding.
```

한국어 메시지:

```text
바이브코딩을 처음 하는 사람도 PRD, 기술 설계, API, 데이터, QA를 같은 지도 위에서 보고 고칠 수 있게 만드는 문서 운영 레이어.
```
...
| 리스크 | 설명 | 대응 |
|---|---|---|
| 범위 과확장 | LazyCodex처럼 agent harness가 되려 하면 구현/검증 범위가 폭발함 | documentation operating layer로 제한 |
...
- "Dreampia는 LazyCodex의 문서 버전"이라는 설명이 통한다.
- 하지만 "Dreampia는 구현 오케스트레이터가 아니라 문서 운영 레이어"라는 차이가 분명하다.
