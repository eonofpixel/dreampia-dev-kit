...
const documentRules = {
  ...
  ia: {
    ...
    patterns: {
      "screen IDs": /PAGE-[A-Z0-9-]+/,
      routes: /`\/[^`]+`|\/[a-z0-9][a-z0-9/:_-]*/i,
      "access roles": /\b(Guest|User|Admin|Owner|Viewer|Editor)\b/i,
    },
  },
  "user-flow": {
    prefix: "FLOW",
    ...
    patterns: {
      "step markers": /\b(Step\s+\d+|\d+\.)\b/i,
      "screen references": /PAGE-[A-Z0-9-]+/,
      "state transitions": /\b(from|to|event|state|transition)\b/i,
    },
  },
  "qa-checklist": {
    prefix: "QA",
  ...
  "doc-audit-report": {
    ...
    patterns: {
      severities: /\b(Critical|Major|Minor)\b/,
      "affected docs": /\b(PRD|TRD|IA|FLOW|API|ERD|QA)-[A-Z0-9-]+-\d{3}\b|docs\//,
      "recommended fixes": /\b(recommend|fix|action|update)\b/i,
    },
  },
};
