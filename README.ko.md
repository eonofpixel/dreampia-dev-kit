...
`orchestrate`와 alias인 `doc-ops`는 읽기 전용 문서 운영 큐를 만듭니다. 빠진 문서, 구조 점수, content-risk, 구현 준비 상태, 다음 CLI 명령, Codex/Claude Code prompt를 한 번에 보여줍니다.
...
전체 docs 폴더에서 다음 작업을 고르려면 아래 명령을 사용합니다.

```bash
node bin/dreampia-dev-kit.js doc-ops --mode expert docs/
```

문서 운영 리포트는 읽기 전용입니다. 문서를 고쳐야 하는지, 검증을 다시 돌릴지, minor risk를 정리할지, 가장 작은 구현 slice로 넘어갈지 판단합니다.
